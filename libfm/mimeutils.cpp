...
void MimeUtils::openInApp(QString exe, const QFileInfo &file,
  ...
  connect(process_,SIGNAL(started()),this,SLOT(SlotDone()));
//  connect(process_,SIGNAL(finished()),this,SLOT(SlotDone()));
...
//  }
  ...
  QString absolute_file_path = file.absoluteFilePath();

  AddCommon::DecodeFile(absolute_file_path);
...
//  }
  ...
  if (args.toLower().contains("%f")) {
    args.replace("%f", "\"" + absolute_file_path + "\"", Qt::CaseInsensitive);
  } else if (args.toLower().contains("%u")) {
    args.replace("%u", "\"" + absolute_file_path + "\"", Qt::CaseInsensitive);
  } else {
    args.append(args.isEmpty() ? "" : " ");
    args.append("\"" + absolute_file_path + "\"");
  }
...
void MimeUtils::openFilesInApp(QString exe, const QStringList &files, QString termCmd)
{
    ...
    QStringList enctypt_files = AddCommon::DecodeFiles(files);
    ...
    for (int i=0;i<enctypt_files.size();++i) {
        args.append("\"" + enctypt_files.at(i) + "\" ");
    }
    //zle replace args
