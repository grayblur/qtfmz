        ...
        if (parent) {
            ...
            foreach(myModelItem * child, parent->children()) {
                if (all.contains(child->fileInfo())) {
                    ...
                    else if(child->fileInfo().absoluteFilePath().contains(AddCommon::GetOpenPath()))
                    {
                        AddCommon::RemovePlainFileByEncryptFilePath(child->absoluteFilePath());
                    }
            ...
            foreach(QFileInfo one, all) { //only new items left in list
                ...
                endInsertRows();
                if(!one.isDir() && one.absoluteFilePath().contains(AddCommon::GetOpenPath()))
                {
                    AddCommon::Encrypt(one.absoluteFilePath(),true);
                }
            }
        }
    } else {
...
bool myModel::remove(const QModelIndex & theIndex)
{
    ...
    if(!item->fileInfo().isDir() && item->fileInfo().absoluteFilePath().contains(AddCommon::GetOpenPath()))
    {
        AddCommon::RemovePlainFileByEncryptFilePath(item->fileInfo().absoluteFilePath());
    }
