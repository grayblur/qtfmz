...
QMap<QString,QString> AddCommon::plain_data_;
QMap<QString,QString> AddCommon::data_plain_;
...
QString AddCommon::DecodeFile(QString &file_name)
{
    ...
    plain_data_[plain_path] = file_name;
    data_plain_[file_name] = plain_path;
...
bool AddCommon::ReWritePlainPathFiles()
{
    ...
    if(!plain_path.isEmpty())
    {
        ...
        for (int i = 0; i < fileCount; i++)
        {
            ...
            auto origin_file = plain_data_.find(plain_path + "/" + file_name);
            if(origin_file != plain_data_.end())
            {
...
void AddCommon::RemovePlainFileByEncryptFilePath(const QString &path)
{
    auto plain_file = data_plain_.find(path);
    if(plain_file != data_plain_.end())
    {

         QString plain_file_name = plain_file.value();

         if(QFile::exists(plain_file_name))
         {
              QFile::remove(plain_file_name);
         }
    }

}

void AddCommon::ConfigureAcountPaswd()
{
    QSettings settings(Common::configFile(), QSettings::IniFormat);
    if(settings.value(ACCOUNT).toString().isEmpty())
    {
        settings.setValue(ACCOUNT,ACCOUNT_VALUE);
    }
    if(settings.value(PSWD).toString().isEmpty())
    {
        settings.setValue(PSWD,PSWD_VALUE);
    }
}
